...
export function NotchNav({
  ...
  const focusItem = (index: number) => {
    itemRefs.current[Math.max(0, Math.min(items.length - 1, index))]?.focus()
  }
  ...
  return (
    <nav aria-label={ariaLabel} className={["w-fit mx-auto", className].filter(Boolean).join(" ")}>
      <div ref={containerRef} className="relative rounded-lg border border-gray-200 bg-gray-100 text-gray-900">
        <ul
          role="menubar"
